...
namespace Proyecto_InefableMedia.Forms
{
    public partial class Registro_usuarios : Form
    {
        ...
        public static bool validarAFP(string AFP)
        {
            string afp = "[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]";
            if(Regex.IsMatch(AFP, afp))
            {
                if(Regex.Replace(AFP, afp, string.Empty).Length == 0)
                {
                    return true;
                }
                else
        ...
        private void txt_numafp_Leave(object sender, EventArgs e)
        {
            if (validarAFP(txt_numafp.Text))
            {

            }
            else
            {
                MessageBox.Show("Formato de numero de AFP, incorrecto", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txt_numafp.SelectAll();
                txt_numafp.Clear();
            }
        }
    }
